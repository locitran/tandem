...
def use_all_gpus():
    ...
    # Log gpus
    logging.error("Num GPUs Available: %d", len(gpus)) # Write to log
    logging.error("GPUs: %s", gpus) # Write to log
    if gpus:
        try:
            for gpu in gpus:
                tf.config.experimental.set_memory_growth(gpu, True)
        except RuntimeError as e:
            logging.error(e) # Write to log
...
def train_model(train_ds, val_ds, cfg, folder, filename, 
    ...
    if fold_num:
        model.save(os.path.join(folder, f'{filename}_{fold_num}_initial.h5'))
...
def reproduce_foundation_model(name='7to8'):
    """
    ...
    for i, fold in folds.items():
        ...
        msg = "Fold %d - val_loss: %.2f, val_accuracy: %.1f%%, val_auc: %.2f, val_precision: %.2f, val_recall: %.2f, val_f1: %.2f, " + \
            ...
            "RYR1_loss: %.2f, RYR1_accuracy: %.1f%%, RYR1_auc: %.2f, RYR1_precision: %.2f, RYR1_recall: %.2f, RYR1_notnan_f1: %.2f, " + \
            "GJB2_loss: %.2f, GJB2_accuracy: %.1f%%, GJB2_auc: %.2f, GJB2_precision: %.2f, GJB2_recall: %.2f, GJB2_notnan_f1: %.2f"
        logging.error(
            msg, i+1, 
            val_rs[0], val_rs[1] * 100, val_rs[2], val_rs[3], val_rs[4], val_rs[5],
            test_rs[0], test_rs[1] * 100, test_rs[2], test_rs[3], test_rs[4], test_rs[5], 
            RYR1_notnan_rs[0], RYR1_notnan_rs[1] * 100, RYR1_notnan_rs[2], RYR1_notnan_rs[3], RYR1_notnan_rs[4], RYR1_notnan_rs[5],
            GJB2_notnan_rs[0], GJB2_notnan_rs[1] * 100, GJB2_notnan_rs[2], GJB2_notnan_rs[3], GJB2_notnan_rs[4], GJB2_notnan_rs[5]
        )
        evaluations[i] = {
...
def reproduce_transfer_learning_model(
    ...
    for split_idx in range(3):
        ...
        model_dir = f'{log_dir}/split_{split_idx}'
        ...
        for model_idx in range(5):
            ...
            model = train_model(
                ...
                cfg=cfg,
                folder=model_dir, 
                filename=f'{model_idx+1}',
                model_input=fd_model_cp,
            )
            ### Evaluation before training
            before_R20000_val_performance = fd_model.evaluate(R20000_val_ds)
            before_R20000_test_performance = fd_model.evaluate(R20000_test_ds)
            before_val_performance = fd_model.evaluate(val_ds)
            before_test_performance = fd_model.evaluate(test_ds)
            before_knw_performance = fd_model.evaluate(knw_ds)
